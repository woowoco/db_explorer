  ...
  Map<String, Object?> _profileToJson(DatabaseConnectionConfig profile) {
    return switch (profile) {
      ...
      RedisConnectionProfile() => {
        ...
        'options': profile.options,
        'password': profile.password,
        'dbIndex': profile.dbIndex,
        'useTls': profile.useTls,
        'connectTimeoutSeconds': profile.connectTimeoutSeconds,
      },
      ElasticsearchConnectionProfile() => {
        'kind': 'elasticsearch',
        'id': profile.id,
        'label': profile.label,
        'host': profile.host,
  ...
  DatabaseConnectionConfig _profileFromJson(Map<String, Object?> json) {
    ...
    return switch (kind) {
      ...
      'redis' => RedisConnectionProfile(
        ...
        password: json['password'] as String?,
        username: json['username'] as String?,
        dbIndex: (json['dbIndex'] as int?) ?? 0,
        useTls: (json['useTls'] as bool?) ?? false,
        connectTimeoutSeconds: json['connectTimeoutSeconds'] as int?,
        options: (json['options'] as Map?)?.cast<String, String>() ?? const {},
      ),
      'elasticsearch' => ElasticsearchConnectionProfile(
        id: json['id'] as String,
        label: json['label'] as String,
        host: json['host'] as String,
        port: (json['port'] as int?) ?? 9200,
